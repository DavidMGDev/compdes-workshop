# =============================================================================
#  onyx.ps1 — Ruta A: desplegar Onyx Lite y darle poder (Windows PowerShell)
# =============================================================================
...
# El archivo de modo Lite podría cambiar de nombre entre versiones de Onyx.
$LitePath = Join-Path $Compose $Lite
if (-not (Test-Path $LitePath)) {
  Write-Host "    FALTA '$Lite' en $Compose"
  ...
  Write-Host "      Ajuste la variable `$Lite en este script o vea docs\ONYX.md."
  exit 1
}

# --- Paso 3: levantar Onyx Lite -------------------------------------------
Write-Host "`n==> [3/6] Levantando Onyx Lite (la primera vez descarga cientos de MB)..."
...
docker compose -f docker-compose.yml -f $Lite up -d
Pop-Location
Write-Host "    OK Onyx arrancando. Tarda ~1-2 min en estar listo en http://localhost:3000"
